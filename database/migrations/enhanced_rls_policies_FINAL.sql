-- 🛡️ Enhanced RLS Policies for Beta Security (FINAL FIXED VERSION)
-- This script adds missing policies and fixes security gaps
-- REMOVED auth schema functions to fix permission issues
...
-- Prevent duplicate reviews for the same appointment (FIXED SYNTAX)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'reviews' 
        AND constraint_name = 'unique_review_per_appointment'
    ) THEN
        ALTER TABLE public.reviews ADD CONSTRAINT unique_review_per_appointment 
...
-- ========================================
-- 8. DATA INTEGRITY CONSTRAINTS (FIXED)
-- ========================================
...
BEGIN
    IF NOT EXISTS (
        ...
        AND constraint_name = 'appointment_future_date'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        AND constraint_name = 'positive_price'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'services' 
        AND constraint_name = 'reasonable_duration'
    ) THEN
        ALTER TABLE public.services 
