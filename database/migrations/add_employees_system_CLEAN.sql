-- Migration: Add Employee System (Clean Version)
-- This adds the ability for businesses to have multiple employees
-- Schema verified against your actual database-schema.sql
...
-- 4. Add employee_id to reviews table
...
-- 6. Create triggers for updated_at columns (using existing function)
...
-- 8. Create RLS policies for employees table
...
-- Allow clients to view active employees (using profiles.role)
...
-- 9. Create RLS policies for employee_availabilities table
...
-- 10. Create function to automatically create owner as employee when provider is created
...
BEGIN
  ...
  ELSE
    -- Return provider's availability (using correct weekday column)
    RETURN QUERY
...
-- Success message
SELECT 'Employee System Migration Completed Successfully!' as status;